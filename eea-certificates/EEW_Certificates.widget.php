 ...
 *
 * EEW_Certificates
 * Displays a List of Certificates in the Sidebar
 *
 * @package			Event Espresso
 ...
 * ------------------------------------------------------------------------
 */
class EEW_Certificates extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'ee-certificates-widget',
			__( 'Event Espresso Certificates Widget', 'event_espresso' ),
			 array(
			 	'description' => __( 'Displays Espresso Certificates in a widget.', 'event_espresso' )
			 ),
			array(
				'width' => 300,
				'height' => 350,
				'id_base' => 'ee-certificates-widget'
			)
		);
	}
...
<?php
	...
	public function widget( $args, $instance ) {
		...
		if ( isset( $post->post_content )) {
			 ...
			 if ( strpos( $post->post_content, '[ESPRESSO_CERTIFICATES') === FALSE ) {
				...
				// load scripts
				EE_Certificates::instance()->enqueue_scripts();
				// settings
				$attributes = array();
				echo EE_Certificates::instance()->display_certificates( $attributes );
...
// End of file EEW_Certificates.widget.php
// Location: /wp-content/plugins/eea-certificates/EEW_Certificates.widget.php