...
class EE_Breakouts_Main {
    // This property will hold the page id for the breakout page (set via options) @var int
    private $_breakout_page;
    // holds the url for the breakout page (derived as the permalink from what is in $_breakout_page); @var string
    private $_breakout_url;
    // This will hold the settings for the plugin. @var array
    private $_settings = array();
    // This will hold the event categories that hold the events in breakout sessions. @var array
    private $_breakout_categories = array();
    // This array holds the page routes array (which directs to the appropriate function when an route is requested). @var array
    private $_page_routes = array();
    // holds the current route @var string
    private $_route = '';
    // This just holds any transient data passed between routes @var array
    private $_route_transient = array();
    // This simply lets us know if we're loading a UI or not. @var boolean
    private $_is_UI = TRUE;
    // This holds all the values that get used in a template. defaults are set in _set_props() method. @var array
    private $_template_args = array();
    // This property will hold the output of the template parser for use in replacing the breakout shortcode. @var string
    private $_content = '';
    // This flags whether this is an AJAX request or not. @var boolean
    private $_doing_ajax = FALSE;
    // This will hold all the request data incoming @var array
    private $_req_data = array();
    // This holds the $_SESSION array for easier retrieval through the class. @var array
    ...
    // start the breakout session. @access public @return void
    ...
    // verifies the entered registration id and if it is valid then we set continue the session and go to the breakout registration form. @access private @return void
    ...
    // This displays the breakout details. @access private @return void
    private function _display_breakouts() {
        $this->_route = 'breakout_registration';
        $this->_display_breakout_registration();
    }
    /**
     * This displays the breakout registration form NOTE: we're just going to collect basic info on the breakout registration and for now this is not customizable.
     * @access private
    ...
    private function _display_breakout_registration() {
        ...
        //setup fields, need to get the breakout categories and then get the breakout events in each category
        $category_name = '';
    ...
    // processed breakout registration form and then redirects accordingly. @access private @return void
    ...
    // This just checks to see if we have a "Breakout Session..." price type setup for Breakout sessions.  If we don't, then we create it.  If we do then we'll just use whatever is set.  This price type will assume the title of the Breakout Session Name @return void
     ...
     * Current inactive limit is set at 1 hour. @return void
     */
