 ...
 * ------------------------------------------------------------------------
 */
class EES_Espresso_Cert extends EES_Shortcode {
    ...
    public function setup_for_load() {
        ...
        if ( EE_Registry::instance()->REQ->is_set( 'resend' ) ) {
            EES_Espresso_Cert::resend_reg_confirmation_email();
        }

        //conditionally load assets
        if ( ! has_action( 'wp_enqueue_scripts', array( 'EES_Espresso_Cert', 'enqueue_styles_and_scripts' ) ) ) {
            add_action( 'wp_enqueue_scripts', array( 'EES_Espresso_Cert', 'enqueue_styles_and_scripts' ) );
        }
    }
    ...
    public static function set_hooks_admin() {
        add_action( 'wp_ajax_ee_my_events_load_paged_template', array( 'EES_Espresso_Cert', 'load_paged_template_via_ajax' ) );
        add_action( 'wp_ajax_nopriv_ee_my_events_load_paged_template', array( 'EES_Espresso_Cert', 'load_paged_template_via_ajax' ) );
    }
    ...
    public static function enqueue_styles_and_scripts() {
        ...
        wp_register_style( 'ees-espresso-cert', EE_WPUSERS_URL . 'assets/css/ees-espresso-my-events.css', array( 'espresso_default' ), EE_WPUSERS_VERSION );
        wp_register_script( 'ees-espresso-cert-js', EE_WPUSERS_URL . 'assets/js/ees-espresso-my-events.js', array( 'espresso_core' ), EE_WPUSERS_VERSION, true );
        wp_enqueue_style( 'ees-espresso-cert' );
        wp_enqueue_script( 'ees-espresso-cert-js' );
    }
    ...
    /**
     * process_shortcode - ESPRESSO_CERT - Return a list of event and registration information specific to the
     * logged in user.
     *
     * [ESPRESSO_CERT] - defaults to use the "event_section" layout for the returned events.
     * [ESPRESSO_CERT template="simple_list_table"] - the value for the template param will load a template matching
     *                                                  the given slug.
     * [ESPRESSO_CERT your_events_title="Your Classes"] - a way to modify the default "Your Events" title to
     *                                                          something different
     * [ESPRESSO_CERT your_tickets_title="Your Tokens"] - a way to modify the default "Your Tickets" title to
     *                                                          something different.
     * [ESPRESSO_CERT per_page=10] - use this to indicate the number of records per page shown ("records" is
    ...
    public function process_shortcode( $attributes = array() ) {
        ...
        if ( apply_filters( 'FHEE__fallback_shortcode_processor__EES_Espresso_Events', false ) ) {
            if ( WP_DEBUG ) {
                return '<div class="important-notice ee-attention">'
                       . __( 'The [ESPRESSO_CERT] is not supported outside of post content fields at this time.', 'event_espresso' )
                       . '</div>';
            } else {
                return '';
            }
        ...
        if ( ! is_user_logged_in() ) {
            $redirect_url = EES_Espresso_Cert::get_current_page();
            return apply_filters(
                'FHEE__Espresso_Cert__process_shortcode__redirect_to_login_instructions',
    ...
    public static function load_paged_template_via_ajax() {
        ...
        /** @type EES_Espresso_Cert $shortcode */
        $shortcode = EES_Espresso_Cert::instance();
    ...
    public function load_template( $attributes = array(), $with_wrapper = true ) {
        ...
        add_filter( 'FHEE__EEH_Template__locate_template__template_folder_paths', array( 'EES_Espresso_Cert', 'template_paths' ), 10 );

        //set default attributes and filter
        $default_shortcode_attributes = apply_filters(
            'FHEE__EES_Espresso_Cert__process_shortcode__default_shortcode_atts',
            array(
    ...
    public function _enqueue_localized_js_object( $attributes ) {
        ...
        wp_localize_script( 'ees-espresso-cert-js', 'EE_MYE_JS', $js_object );
    }
    ...
    protected function _get_template_object_map() {
         ...
         * Once custom templates can be registered, then they can be utilized in the `[Cert]`
         * shortcode via the "template" parameter.
         */
        return apply_filters(
            'FHEE__EES_Espresso_Cert__process_shortcode_template_object_map',
            array(
                'simple_list_table' => array(
                    'object_type' => 'Registration',
                    'path' => 'loop-espresso_cert-simple_list_table.template.php',
                ),
                'event_section' => array(
                    'object_type' => 'Event',
                    'path' => 'loop-espresso_cert-event_section.template.php'
                )
            )
        );
    }
    ...
    public static function resend_reg_confirmation_email() {
        ...
        if ( EE_FRONT_AJAX ) {
            ...
            wp_safe_redirect(
                    EES_Espresso_Cert::get_current_page()
                );
        }
    }

} //end EES_Espresso_Cert
