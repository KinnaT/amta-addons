 ...
 *
 * EEW_Cert
 * Displays a List of Cert in the Sidebar
 *
 * @package            Event Espresso
 * @subpackage     eea-cert
 ...
 * ------------------------------------------------------------------------
 */
class EEW_Cert extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            'ee-cert-widget',
            __( 'Event Espresso Cert Widget', 'event_espresso' ),
             array(
                 'description' => __( 'Displays Espresso Cert in a widget.', 'event_espresso' )
             ),
            array(
                'width' => 300,
                'height' => 350,
                'id_base' => 'ee-cert-widget'
            )
        );
    }
...
<?php
    ...
    public function widget( $args, $instance ) {
        ...
        if ( isset( $post->post_content )) {
             ...
             if ( strpos( $post->post_content, '[ESPRESSO_CERT') === FALSE ) {
                ...
                // load scripts
                EE_Cert::instance()->enqueue_scripts();
                // settings
                $attributes = array();
                echo EE_Cert::instance()->display_cert( $attributes );
...
// End of file EEW_Cert.widget.php
// Location: /wp-content/plugins/eea-cert/EEW_Cert.widget.php
