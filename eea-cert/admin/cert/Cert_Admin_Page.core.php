...
class Cert_Admin_Page extends EE_Admin_Page {
    ...
    protected function _set_page_config() {
        ...
        $this->_page_config = array(
            ...
            'usage' => array(
                'nav' => array(
                    'label' => __('Cert Usage', 'event_espresso'),
                    'order' => 30
                    ),
                'require_nonce' => FALSE
            )
    ...
    public function load_scripts_styles() {
        wp_register_script( 'espresso_cert_admin', EE_CERT_ADMIN_ASSETS_URL . 'espresso_cert_admin.js', array( 'espresso_core' ), EE_CERT_VERSION, TRUE );
        wp_enqueue_script( 'espresso_cert_admin');
    }

    public function admin_init() {
        EE_Registry::$i18n_js_strings[ 'confirm_reset' ] = __( 'Are you sure you want to reset ALL your Event Espresso Cert Information? This cannot be undone.', 'event_espresso' );
    }
    ...
    protected function _basic_settings() {
        $this->_settings_page( 'cert_basic_settings.template.php' );
    }
    ...
    protected function _settings_page( $template ) {
        $this->_template_args['cert_config'] = EE_Config::instance()->get_config( 'addons', 'EED_Cert', 'EE_Cert_Config' );
    ...
    protected function _usage() {
        $this->_template_args['admin_page_content'] = EEH_Template::display_template( EE_CERT_ADMIN_TEMPLATE_PATH . 'cert_usage_info.template.php', array(), TRUE );
        $this->display_admin_page_with_no_sidebar();
    }

    protected function _update_settings(){
        if(isset($_POST['reset_cert']) && $_POST['reset_cert'] == '1'){
            $config = new EE_Cert_Config();
            $count = 1;
        }else{
            $config = EE_Config::instance()->get_config( 'addons', 'EED_Cert', 'EE_Cert_Config' );
            $count=0;
            //otherwise we assume you want to allow full html
            foreach($this->_req_data['cert'] as $top_level_key => $top_level_value){
        ...
        EE_Config::instance()->update_config( 'addons', 'EED_Cert', $config );
...
// End of file Cert_Admin_Page.core.php
// Location: /wp-content/plugins/eea-cert/admin/cert/Cert_Admin_Page.core.php
